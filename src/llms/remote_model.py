...
class GPT(BaseLLM):
    ...
    def request(self, query: str) -> str:
        ...
        res = res.json()
        real_res = res["data"]["response"]["choices"][0]["message"]["content"]

        token_consumed = res["data"]["total_tokens"]
        logger.info(f'GPT token consumed: {token_consumed}') if self.report else ()
        return real_res

class GPT4_A(BaseLLM):
    def __init__(self, model_name='gpt-4', temperature=1.0, max_new_tokens=1024, report=False):
        super().__init__(model_name, temperature, max_new_tokens)
        self.report = report
    def request(self, query: str) -> str:
        ...
        token_consumed = res["data"]["response"]["choices"][0]['total_tokens']
        logger.info(f'GPT token consumed: {token_consumed}') if self.report else ()
        return real_res
